...
import { evaluateQuery, extractDateBounds } from '../dsl/executor';
import { parseQuery } from '../dsl/parser';
import { QueryPlayground } from '../components/playground';
import type { Route } from './+types/home';

export function meta({}: Route.MetaArgs) {
  return [{ title: 'Query Playground' }, { name: 'description', content: 'Query playground' }];
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    if (!ast) {
      return Response.json({ error: 'Failed to parse query' }, { status: 400 });
    }
