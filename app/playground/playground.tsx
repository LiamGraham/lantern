...
import { validateQuery } from "../dsl/service";
...
export function QueryPlayground() {
	...
	const buildFinalQuery = () => {
		...
		if (excludeTransfers && finalQuery) {
			finalQuery = `${finalQuery} !type:transfer`;
		} else if (excludeTransfers) {
			finalQuery = "!type:transfer";
		}
			...
			<div className="space-y-4">
				...
				<div className="space-y-3">
					...
					{finalQuery !== query.trim() && (
						<div className="text-xs bg-neutral-600 px-3 py-2 rounded border border-neutral-600">
							<span className="font-medium">Final query:</span>{" "}
							<code className="font-mono">{finalQuery}</code>
						</div>
					)}
				</div>
			</div>
					...
					<div className="bg-neutral-600 border border-neutral-600  rounded-md p-4">
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
							<div>
								...
								{queryResult.executionInfo.totalFetched}
							</div>
							<div>
								<span className="font-medium">Total Filtered:</span>{" "}
								{queryResult.executionInfo.totalFiltered}
							</div>
							<div>
								<span className="font-medium">Date Bounds:</span>{" "}
								{queryResult.executionInfo.dateBounds.since ||
								queryResult.executionInfo.dateBounds.until
									? `${queryResult.executionInfo.dateBounds.since ? new Date(queryResult.executionInfo.dateBounds.since).toLocaleDateString() : "No start"} - ${queryResult.executionInfo.dateBounds.until ? new Date(queryResult.executionInfo.dateBounds.until).toLocaleDateString() : "No end"}`
									: "No date filters"}
							</div>
						</div>
					</div>
			...
			<div className="space-y-2">
				...
				<div className="flex flex-wrap gap-2">
					{[
						...
						"amount:10..100 AND status:settled",
						'description:"*payment to*" !type:transfer',
						"category:food* amount:>50 since:7d",
					].map((example) => (
						<button
							type="button"
							key={example}
